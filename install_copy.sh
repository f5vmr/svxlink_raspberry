...
sudo sh -c 'echo $(date) ${YELLOW} "########## Downloading SVXLink source code ##########" ${NORMAL} >> /var/log/install.log'
source "${BASH_SOURCE%/*}/functions/source.sh"
#svxlink_source	

#	NEWVERSION= `sudo grep "SVXLINK=" $VERSIONS | awk -F= '{print $2}'
	sudo sh -c 'echo $(date) New Version: $NEWVERSION  >> /var/log/install.log'
sleep 3
########## INSTALLATION ##########	
########## COMPILING ##########
	sudo sh -c 'echo $(date) ${YELLOW} "########## Compiling ##########" ${NORMAL} >> /var/log/install.log'
sleep 3
#	cd svxlink/src/build
#	sudo cmake -DUSE_QT=OFF -DCMAKE_INSTALL_PREFIX=/usr -DSYSCONF_INSTALL_DIR=/etc -DLOCAL_STATE_DIR=/var -DWITH_SYSTEMD=ON  ..
#	sudo make
#	sudo make doc
#	sudo sh -c 'echo $(date) ${GREEN}"########## Installing SVXLink ##########" ${NORMAL} >> /var/log/install.log'
#	sudo make install
...
########### CONFIGURATION VOICES ##########
	sudo sh -c 'echo $(date) ${GREEN} "########## Installing Voice Files ##########" ${NORMAL} >> /var/log/install.log'
sleep 3
...
########### BACKUP CONFIGURATION ##########
	sudo sh -c 'echo $(date) backing up configuration to : $CONF.bak >> /var/log/install.log'
sleep 3
#	sudo cp -p $CONF $CONF.bak
##
#	cd
#	sudo sh -c 'echo $(date) ${RED} "########## Downloading prepared configuration files from the scripts ##########" ${NORMAL} >> /var/log/install.log'
...
#	(sudo crontab -l 2>/dev/null; echo "59 23 * * * /home/pi/scripts/resetlog.sh ") | sudo crontab -
	...
	sudo sh -c 'echo $(date) ${GREEN} Setting Callsign to "$CALL"${NORMAL} >> /var/log/install.log'
sleep 3
#	sudo sed -i "s/MYCALL/$CALL/g" $CONF
#	sudo sed -i "s/MYCALL/$CALL/g" /etc/svxlink/node_info.json

#	sudo sh -c 'echo $(date) Setting Squelch Hangtime to 10 >> /var/log/install.log'
#	sudo sed -i "s/SQL_HANGTIME=2000/SQL_HANGTIME=10/g" $CONF
#	
	sudo sh -c 'echo $(date) Disabling audio distortion warning messages >> /var/log/install.log'
sleep 3
#	sudo sed -i "s/PEAK_METER=1/PEAK_METER=0/g" $CONF
#
	sudo sh -c 'echo $(date) Updating SplashScreen on startup >> /var/log/install.log'
sleep 3
#	sudo sed -i "s/MYCALL/$CALL/g" /etc/update-motd.d/10-uname
#	sudo chmod 0755 /etc/update-motd.d/10-uname
#
	sudo sh -c 'echo $(date) Changing Log file >> /var/log/install.log'
sleep 3
#	sudo sed -i "s/log\/svxlink/log\/svxlink.log/g" /etc/default/svxlink
	########## INSTALLING DASHBOARD ##########
	sudo sh -c 'echo $(date) ${YELLOW} "######## Installing Dashboard ########" ${NORMAL} >> /var/log/install.log'
sleep 3
#	if [ $card=true ] ;
#	then
...
#	sudo sed -i "s/\#DEFAULT_LANG=en_US/DEFAULT_LANG=en_GB/g" /etc/svxlink/svxlink.d/ModuleMetarInfo.conf	
 	sudo sh -c 'echo{RED}Changing ModuleMetar Link${NORMAL} >> /var/log/install.log'
sleep 3
#  	sudo sed -i "s%#LINK=data/observations/metar/stations%LINK=/cgi-bin/data/dataserver.php?requestType=retrieve&dataSource=metars&hoursBeforeNow=3&stationString=%g" /etc/svxlink/svxlink.d/ModuleMetarInfo.conf
	sudo sh -c 'echo $(date) ${RED} Authorise GPIO setup service (Unused) and svxlink service${NORMAL} >> /var/log/install.log'
sleep 3
...
sudo sh -c 'echo $(date) ${RED}Installation complete\n${NORMAL} >> /var/log/install.log'
sleep 3
sudo sh -c 'echo $(date) ${GREEN} Reboot - Redémarrer SVXLink\n\n\n${NORMAL} >> /var/log/install.log'
sleep 3
exit
