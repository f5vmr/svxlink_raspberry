...
########## GROUPS AND USERS ##########
clear
echo -e "$(date)" "${YELLOW} Creating Groups and Users ${NORMAL}" | tee /var/log/install.log
source "${BASH_SOURCE%/*}/functions/groups.sh"
#make_groups

########## DOWNLOADING SOURCE CODE ##########

echo -e "$(date)" "${YELLOW} ########## Downloading SVXLink source code ########## ${NORMAL}" | tee /var/log/install.log
source "${BASH_SOURCE%/*}/functions/source.sh"
#svxlink_source	

#	NEWVERSION= `sudo grep "SVXLINK=" $VERSIONS | awk -F= '{print $2}'
	echo -e "$(date)" New Version: $NEWVERSION  | tee /var/log/install.log
sleep 3
########## INSTALLATION ##########	
########## COMPILING ##########
	echo -e "$(date)" "${YELLOW} ########## Compiling ########## ${NORMAL}" | tee /var/log/install.log
sleep 3
#	cd svxlink/src/build
#	sudo cmake -DUSE_QT=OFF -DCMAKE_INSTALL_PREFIX=/usr -DSYSCONF_INSTALL_DIR=/etc -DLOCAL_STATE_DIR=/var -DWITH_SYSTEMD=ON  ..
#	sudo make
#	sudo make doc
#	echo -e "$(date)" "${GREEN} ########## Installing SVXLink ########## ${NORMAL}" | tee /var/log/install.log
#	sudo make install
...
########### CONFIGURATION VOICES ##########
	echo -e "$(date)" "${GREEN} ########## Installing Voice Files ########## ${NORMAL}" | tee /var/log/install.log
sleep 3
...
########### BACKUP CONFIGURATION ##########
	echo -e "$(date)" backing up configuration to : $CONF.bak | tee /var/log/install.log
sleep 3
#	sudo cp -p $CONF $CONF.bak
##
#	cd
#	echo -e "$(date)" "${RED} ########## Downloading prepared configuration files from the scripts ########## ${NORMAL}" | tee /var/log/install.log
...
#	(sudo crontab -l 2>/dev/null; echo "59 23 * * * /home/pi/scripts/resetlog.sh ") | sudo crontab -
	...
	echo -e "$(date)" "${GREEN} Setting Callsign to "$CALL"${NORMAL}" | tee /var/log/install.log
sleep 3
#	sudo sed -i "s/MYCALL/$CALL/g" $CONF
#	sudo sed -i "s/MYCALL/$CALL/g" /etc/svxlink/node_info.json

#	echo -e "$(date)" Setting Squelch Hangtime to 10 | tee /var/log/install.log
#	sudo sed -i "s/SQL_HANGTIME=2000/SQL_HANGTIME=10/g" $CONF
#	
	echo -e "$(date)" Disabling audio distortion warning messages | tee /var/log/install.log
sleep 3
#	sudo sed -i "s/PEAK_METER=1/PEAK_METER=0/g" $CONF
#
	echo -e "$(date)" Updating SplashScreen on startup | tee /var/log/install.log
sleep 3
#	sudo sed -i "s/MYCALL/$CALL/g" /etc/update-motd.d/10-uname
#	sudo chmod 0755 /etc/update-motd.d/10-uname
#
	echo -e "$(date)" Changing Log file | tee /var/log/install.log
sleep 3
#	sudo sed -i "s/log\/svxlink/log\/svxlink.log/g" /etc/default/svxlink
	########## INSTALLING DASHBOARD ##########
	echo -e "$(date)" "${YELLOW} ######## Installing Dashboard ######## ${NORMAL}" | tee /var/log/install.log
sleep 3
#	if [ $card=true ] ;
#	then
...
#	sudo sed -i "s/\#DEFAULT_LANG=en_US/DEFAULT_LANG=en_GB/g" /etc/svxlink/svxlink.d/ModuleMetarInfo.conf	
 	echo -e "$(date)" "${RED}Changing ModuleMetar Link ${NORMAL}" | tee /var/log/install.log
sleep 3
#  	sudo sed -i "s%#LINK=data/observations/metar/stations%LINK=/cgi-bin/data/dataserver.php?requestType=retrieve&dataSource=metars&hoursBeforeNow=3&stationString=%g" /etc/svxlink/svxlink.d/ModuleMetarInfo.conf
	echo -e "$(date)" "${RED} Authorise GPIO setup service - no longer used and svxlink service${NORMAL}" | tee /var/log/install.log
sleep 3
...
echo -e $(date) "${RED} Installation complete ${NORMAL}" | tee /var/log/install.log
sleep 3 
echo -e $(date) "${GREEN} Rebooting SVXLink ${NORMAL}" | tee /var/log/install.log
sleep 3
exit
