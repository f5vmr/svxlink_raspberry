...
source "${BASH_SOURCE%/*}/functions/source.sh"
	sudo sh -c 'echo $(date) ${YELLOW} "########## Téléchargements de codesource SVXLink ##########" ${NORMAL} >> /var/log/install.log'
	cd
svxlink_source	
...
########## COMPILING ##########
	sudo sh -c 'echo $(date) ${YELLOW} "########## Compilation ##########" ${NORMAL} >> /var/log/install.log'
	cd svxlink/src/build
	sudo cmake -DUSE_QT=OFF -DCMAKE_INSTALL_PREFIX=/usr -DSYSCONF_INSTALL_DIR=/etc -DLOCAL_STATE_DIR=/var -DWITH_SYSTEMD=ON  ..
	sudo make
	sudo make doc
	sudo sh -c 'echo $(date) ${GREEN}"########## Installation SVXLink ##########" ${NORMAL} >> /var/log/install.log'
	sudo make install
...
########### CONFIGURATION VOICES ##########
	sudo sh -c 'echo $(date) ${GREEN} "########## Installation de dossiers Voix ##########" ${NORMAL} >> /var/log/install.log'
...
########### BACKUP CONFIGURATION ##########
	sudo sh -c 'echo $(date) ${GREEN} backing up configuration to : $CONF.bak ${NORMAL} >> /var/log/install.log'
