...
source "${BASH_SOURCE%/*}/functions/source.sh"
	echo -e "$(date)" "${YELLOW} ########## Téléchargements de codesource SVXLink ########## ${NORMAL}" | tee /var/log/install.log
	cd
svxlink_source	
...
########## COMPILING ##########
	echo -e "$(date)" "${YELLOW} ########## Compilation ########## ${NORMAL}" | tee /var/log/install.log
	cd svxlink/src/build
	sudo cmake -DUSE_QT=OFF -DCMAKE_INSTALL_PREFIX=/usr -DSYSCONF_INSTALL_DIR=/etc -DLOCAL_STATE_DIR=/var -DWITH_SYSTEMD=ON  ..
	sudo make
	sudo make doc
	echo -e "$(date)" "${GREEN} ########## Installation SVXLink ########## ${NORMAL}" | tee /var/log/install.log
	sudo make install
...
########### CONFIGURATION VOICES ##########
	echo -e "$(date)" "${GREEN} ########## Installation de dossiers Voix ########## ${NORMAL}" | tee /var/log/install.log
...
########### BACKUP CONFIGURATION ##########
	echo -e "$(date)" "${GREEN} backing up configuration to : $CONF.bak ${NORMAL}" | tee /var/log/install.log
...
#	sudo cp -f svxlink_raspberry/addons/node_info.json /etc/svxlink/node_info.json
	...
	echo -e "$(date)" "${YELLOW} Setting Squelch Hangtime to 10 ${NORMAL}" | tee /var/log/install.log
	sudo sed -i "s/SQL_HANGTIME=2000/SQL_HANGTIME=10/g" $CONF 
#	
	echo -e "$(date)" "${YELLOW} Disabling audio distortion warning messages ${NORMAL}" | tee /var/log/install.log
	sudo sed -i "s/PEAK_METER=1/PEAK_METER=0/g" $CONF
#
	echo -e "$(date)" "${GREEN} Updating SplashScreen on startup ${NORMAL}"  | tee /var/log/install.log
	sudo sed -i "s/MYCALL/$CALL/g" /etc/update-motd.d/10-uname
	sudo chmod 0755 /etc/update-motd.d/10-uname
#
	echo -e "$(date)" "${GREEN} Changing Logfile suffix ${NORMAL}"| tee /var/log/install.log 		
	...
	sudo sed -i "s/\#DEFAULT_LANG=en_US/DEFAULT_LANG=en_GB/g" /etc/svxlink/svxlink.d/ModuleMetarInfo.conf	
 	echo{RED}Changing ModuleMetar Link${NORMAL}" | tee /var/log/install.log
...
%g" /etc/svxlink/svxlink.d/ModuleMetarInfo.conf
	echo -e "$(date)" "${RED} Authorise GPIO setup service - Unused and svxlink service${NORMAL}" | tee /var/log/install.log
