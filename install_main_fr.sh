...
#	sudo cp -f svxlink_raspberry/addons/node_info.json /etc/svxlink/node_info.json
	...
	echo -e "$(date)" "${YELLOW} Setting Squelch Hangtime to 10 ${NORMAL}" | tee -a  /var/log/install.log
...
#
	...
	sudo sed -i "s/\#DEFAULT_LANG=en_US/DEFAULT_LANG=en_GB/g" /etc/svxlink/svxlink.d/ModuleMetarInfo.conf	
 	echo{RED}Changing ModuleMetar Link${NORMAL}" | tee -a  /var/log/install.log
...
%g" /etc/svxlink/svxlink.d/ModuleMetarInfo.conf
	echo -e "$(date)" "${RED} Authorise GPIO setup service - Unused and svxlink service${NORMAL}" | tee -a  /var/log/install.log
