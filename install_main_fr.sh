...
source "${BASH_SOURCE%/*}/functions/source.sh"
	echo "$(date)" "${YELLOW} "########## Téléchargements de codesource SVXLink ##########" ${NORMAL}" | tee /var/log/install.log
	cd
svxlink_source	
...
########## COMPILING ##########
	echo "$(date)" "${YELLOW} "########## Compilation ##########" ${NORMAL}" | tee /var/log/install.log
	cd svxlink/src/build
	sudo cmake -DUSE_QT=OFF -DCMAKE_INSTALL_PREFIX=/usr -DSYSCONF_INSTALL_DIR=/etc -DLOCAL_STATE_DIR=/var -DWITH_SYSTEMD=ON  ..
	sudo make
	sudo make doc
	echo "$(date)" "${GREEN}"########## Installation SVXLink ##########" ${NORMAL}" | tee /var/log/install.log
	sudo make install
...
########### CONFIGURATION VOICES ##########
	echo "$(date)" "${GREEN} "########## Installation de dossiers Voix ##########" ${NORMAL}" | tee /var/log/install.log
...
########### BACKUP CONFIGURATION ##########
	echo "$(date)" "${GREEN} backing up configuration to : $CONF.bak ${NORMAL}" | tee /var/log/install.log
